...
`streaming/` パッケージは、既存モデルの上にリアルタイム認識を追加します。既定では、VADで区切った発話区間内で特徴を蓄積し、チャンクごとに**エンコーダ全体**を再実行します。切り捨て注意機構（truncated attention）は使用せず、`model.py` の推論動作も変更しません。（実験的な第2のバックエンドについては後述します。）
...
#### チャンクバックエンド（実験的 — ファインチューニング済みチェックポイントが前提）

`StreamingConfig.backend` で認識方式を選択します。

| | `"accumulate"`（既定） | `"chunk"` |
|---|---|---|
| エンコーダ処理 | 伸長する窓に対してエンコーダ全体を再実行 | 各フレームを1回だけストリーミングキャッシュへ投入 |
| コスト | 発話長に比例して増加 | フレームあたり一定（ただし固定コストは大きい） |
| 重み | 公開重みでそのまま動作 | チャンク学習済みチェックポイントを想定 |
| ベンチマーク | 実測済み | 未実測 |

コストの行は漸近的な性質であり、性能保証ではありません。6秒の音声での実測では、チャンクバックエンドの方が**低速**でした（RTF 0.818 対 0.697）。この長さでは accumulate 側の窓が `max_history` に達しないため、フレームあたり一定というチャンク側の利点が働きません。窓が飽和するだけの長い発話でのみ優位性が期待できます。
...
**これは単純な高速化オプションではありません。** 公開重みは全系列 self-attention で学習されており、チャンクバックエンドは学習時とは異なる条件で推論します。そのため `finetune_chunk.sh` によるチャンク学習を行うまで、partial の品質は低下します（場合により大幅に低下します）。`final` はどちらのバックエンドでも影響を受けません。蓄積した発話全体に対するフル推論から生成されるためです。

チャンク形状の既定値（`chunk_pad_left` / `chunk_stride` / `chunk_pad_right` / `chunk_encoder_look_back`）は実測に基づくものではなく、`finetune_chunk.sh` の学習設定の中央エントリを反映したものです。ベンチマークの出発点として扱ってください。

エンコーダ側のチャンクマスク学習経路、形状パラメータの規約、検証済み／未検証の範囲、本格的なファインチューニングの前提条件については [docs/chunk_training.md](docs/chunk_training.md) を参照してください。
